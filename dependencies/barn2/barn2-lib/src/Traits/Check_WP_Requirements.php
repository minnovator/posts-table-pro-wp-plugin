...
namespace Barn2\Plugin\Posts_Table_Pro\Dependencies\Lib\Traits;

use Barn2\Plugin\Posts_Table_Pro\Dependencies\Lib\Admin\Notices;
    ...
    private function check_wp_requirements(string $version, Plugin $plugin) : bool
    {
        ...
        if (\is_admin() && \version_compare($wp_version, $version, '<')) {
            ...
            $admin_notice->add($slug . '_invalid_wp_version', '', \sprintf(
                ...
                __('The %1$s plugin requires WordPress %4$s or greater. Please %2$supdate%3$s your WordPress installation.','posts-table-pro' ),
        ...
        if (!\class_exists('WooCommerce')) {
            if (\is_admin()) {
                ...
                $admin_notice->add(
                    ...
                    \sprintf(__('Please %1$sinstall WooCommerce%2$s in order to use %3$s.','posts-table-pro' ), Util::format_link_open('https://woocommerce.com/', \true), '</a>', $plugin->get_name()),
                    ['type' => 'error', 'capability' => 'install_plugins', 'screens' => ['plugins']]
                );
                $admin_notice->boot();
            }
        ...
        if (\version_compare($woocommerce->version, $version, '<')) {
            if (\is_admin()) {
                ...
                $admin_notice->add(
                    ...
                    \sprintf(__('The %1$s plugin requires WooCommerce %2$s or greater. Please update your WooCommerce setup first.','posts-table-pro' ), $plugin->get_name(), $version),
