...
class License_Key_Setting implements Registerable, License_Setting
{
    ...
    public function process_license_action()
    {
        if ($this->is_license_action(self::ACTIVATE_KEY)) {
            ...
            if (isset($license_setting['license'])) {
                ...
                $this->add_settings_message(__('License key successfully activated.', 'posts-table-pro'), __('There was an error activating your license key.', 'posts-table-pro'), $activated);
            }
        } elseif ($this->is_license_action(self::DEACTIVATE_KEY)) {
            $deactivated = $this->license->deactivate();
            $this->add_settings_message(__('License key successfully deactivated.', 'posts-table-pro'), __('There was an error deactivating your license key, please try again.', 'posts-table-pro'), $deactivated);
        } elseif ($this->is_license_action(self::CHECK_KEY)) {
            $this->license->refresh();
            $this->add_settings_message(__('The license key looks good!', 'posts-table-pro'), __('There\'s a problem with your license key.', 'posts-table-pro'), $this->license->is_active());
        }
    }
    ...
    {
        $setting = ['title' => __('License key', 'posts-table-pro'), 'type' => 'text', 'id' => $this->get_license_setting_name() . '[license]', 'desc' => $this->get_license_description(), 'class' => 'regular-text'];
        if ($this->is_woocommerce) {
    ...
    {
        $buttons = ['check' => $this->license_action_button(self::CHECK_KEY, __('Check', 'posts-table-pro')), 'activate' => $this->license_action_button(self::ACTIVATE_KEY, __('Activate', 'posts-table-pro')), 'deactivate' => $this->license_action_button(self::DEACTIVATE_KEY, __('Deactivate', 'posts-table-pro'))];
